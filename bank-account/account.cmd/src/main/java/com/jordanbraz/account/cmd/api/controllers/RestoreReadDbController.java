...
import com.jordanbraz.account.cmd.api.commands.RestoreReadDbCommand;
...
public class RestoreReadDbController {
    ...
    public ResponseEntity<BaseResponse> restoreReadDb() {
        try {
            ...
            return new ResponseEntity<>(new BaseResponse("Restored Completed"), HttpStatus.CREATED);
        } catch (IllegalStateException e) {
            logger.log(Level.WARNING, MessageFormat.format("Client made a bad request - ${0}", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            var safeErrorMessage = "Error while processing request";
